...
class GA_Wang:
    ...
    def selection(self):
        ...
        rank_probabilities = np.array([1.0 / (i + 1) for i in range(len(sorted_population))])
        rank_probabilities /= rank_probabilities.sum()
        selected_population = []
        for _ in range(self.N):
            selected_idx = np.random.choice(len(sorted_population), p=rank_probabilities)
            selected_population.append(copy.deepcopy(sorted_population[selected_idx]))
