        ...
        for src_node in self.topology_monitor.graph:
            ...
            for dst_node in self.topology_monitor.graph[src_node]:
                if src_node != dst_node:
                    metric[src_node][dst_node] = 1
        alg = YenAlgorithm(metric, self.topology_monitor.switches, src, dst, K)
        paths_0, paths_edges_0, pw_0 = alg.compute_shortest_paths()
        paths, paths_edges, pw = [], [], []
        for i in range(len(pw_0)):
            if pw_0[i] == pw_0[0]:
                paths.append(paths_0[i])
                paths_edges.append(paths_edges_0[i])
                pw.append(pw_0[i])
