...
class PortMonitor(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.bw_limit = get_bw_limit('/home/tin/SDN_PyQt5/topo_mininet/10_nodes.json')
        self.monitor_thread = hub.spawn(self._monitor)
        self.example_thread = threading.Thread(target=self.change)
        self.example_thread.daemon = True  # Optional: makes the thread exit when the main program exits
        ...
        for src in self.topology_monitor.graph:
            ...
            for dst in self.topology_monitor.graph[src]:
                if src != dst:
                    ...
                    speed = 0
                    curr_bw = self.bw_limit[src][dst]
                    rm_bw = 0
                    try:
                        speed = self.throughputs[src][dst]
                        rm_bw = self.get_free_bw(curr_bw, speed)
                    except:
                        rm_bw = curr_bw
                    self.rm_bws[src][dst]= rm_bw
                    self.link_utilizations[src][dst]= round((capacity-curr_bw+speed)/capacity, 1)
