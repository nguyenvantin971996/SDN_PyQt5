    ...
    def _packet_in_handler(self, ev):
        ...
        if tcp_pkt or udp_pkt:
            ...
            if condition:
                ...
                self.paths_dict[(h1[0], h1[1], h2[0], h2[1], src_ip, dst_ip, src_port, dst_port)] = [paths, paths_edges, pw, 0, src_dst]
                out_port = self.install_paths_ip(h1[0], h1[1], h2[0], h2[1], src_ip, dst_ip, paths, pw, 0, tcp_pkt, udp_pkt)
                ...
                for key, value in self.paths_dict.items():
                    if port in key and src_ip_2 == key[4] and dst_ip_2 == key[5]:
                        [paths, paths_edges, pw, index, _] = value
                        next_index = (index+1)%len(paths)
                        out_port = self.install_paths_ip(key[0], key[1], key[2], key[3], key[4], key[5], paths, pw, next_index, tcp_pkt, udp_pkt)
                        self.paths_dict[key][3] = next_index
