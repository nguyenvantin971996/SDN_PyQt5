...
PACKET_SIZE_THRESHOLD = 1024

BANDWIDTH_SENSITIVE_IP = ['10.0.0.2']
LATENCY_SENSITIVE_IP = ['10.0.0.4']
...
class MultiPathRouting(app_manager.RyuApp):
    ...
    def get_new_pw(self, paths, flow_type):
        pw = []
        if flow_type == 'Bandwidth-sensitive':
            rm_bw = self.port_monitor.get_rm_bw()
            for path in paths:
                length = 1000000
                for s1, s2 in zip(path[:-1], path[1:]):
                    if length >= rm_bw[s1][s2]:
                        length = rm_bw[s1][s2]
                pw.append(length)
        else:
            latency = self.delay_monitor.get_latency()
            for path in paths:
                length = 0
                for s1, s2 in zip(path[:-1], path[1:]):
                    length += latency[s1][s2]
                pw.append(1/length)
        total = sum(pw)
        if total == 0:
            return pw
    ...
    def _packet_in_handler(self, ev):
        ...
        if tcp_pkt or udp_pkt:
            ...
            flow_type = None
            if dst_ip in BANDWIDTH_SENSITIVE_IP or src_ip in BANDWIDTH_SENSITIVE_IP:
                flow_type = 'Bandwidth-sensitive'
            else:
                flow_type = 'Latency-sensitive'

            paths, paths_edges, pw = [], [], []
            condition, port, src_ip_2, dst_ip_2 = self.check_key(src_ip, dst_ip, src_port, dst_port)

            if condition:
                paths, paths_edges, pw_0 = self.get_optimal_paths(h1[0], h2[0])
                pw = self.get_new_pw(paths, flow_type)
                src_dst = "Proposed: " + src_ip + " --> " + dst_ip + ', ' + src_port + ' - ' + dst_port + ' ('+ flow_type + ')'
                self.paths_dict[(h1[0], h1[1], h2[0], h2[1], src_ip, dst_ip, src_port, dst_port, flow_type)] = (paths, paths_edges, pw, src_dst)
