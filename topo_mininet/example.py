...
# Класс для определения топологии сети
class MyTopo(Topo):
    ...
    def build(self, **param):
        ...
        # Добавление каналов между узлами и коммутаторами
...
# Функция для создания сети, запуска топологии и работы с CLI
def topology():
    topo = MyTopo()  # Инициализация топологии
    link = partial(TCLink)
    
    # Создание сети с удаленным контроллером, запуск и настройка сети
...
# Главная функция для настройки уровня логов и запуска топологии
if __name__ == '__main__':
    setLogLevel('info')  # Установка уровня логирования
    topology()  # Запуск функции topology